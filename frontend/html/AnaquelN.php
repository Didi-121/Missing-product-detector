...
<head>
  ...
  <link rel="stylesheet" href="../css/anaquelN.css">
  <link rel="stylesheet"
  ...
  <main class="anaquel-main row m-0 col-md-12">
    ...
    <section class="anaquel-info col-md-7">
      ...
      <section class="anaquel-bar">
        ...
        <img src="" id="captura" alt="" class="col-12 my-4" style="display:none; border-radius: 1rem;width: 80%;">
        <div class="row col-md-12 m-0 pb-5 decide" style="display: none; justify-content: space-evenly;">
          <button class="retake-btn col-md-4"><span class="material-symbols-rounded">autorenew</span> Volver a
            tomar</button>
          <button class="confirm-btn col-md-4"><span class="material-symbols-rounded">check</span>Confirmar</button>
        </div>
        <button class="button-tomar-foto" onclick="abrirCamara()">
          <span class="material-symbols-rounded" style="color: #4B5563; font-size: 200px;"> photo_camera</span>
  ...
  <div class="overlap">
    <div class="card p-4 col-md-5 m-auto" style="max-height: 90lvh;">
      ...
      <div id="role-select" class="row">
        <button class="btn-share col-md-5 py-2 mx-auto" onclick="startAsPhone()">Usar este dispositivo como cámara
          (Teléfono)</button>
        <button class="btn-recibe col-md-5 py-2 mx-auto" onclick="startAsViewer()">Ver desde otro dispositivo
          (PC)</button>
      </div>
      ...
      <div id="viewer-section" style="display:none;flex-direction: column;">
        ...
        <button class="solo-camera-icon mx-auto mt-1" style="display: none;"><span class="material-symbols-rounded"
...
<script>
  ...
  $('.retake-btn').click(function () {
    ...
    $('.retake-btn').hide();
    $('.confirm-btn').hide();
    $('.button-tomar-foto').show();
    $('.boton_informativo').show();
  });

  $('.confirm-btn').click(function () {
    $('.anaquel-image').children('.anaquel-title-bar').css('margin', '0px').css('padding', '0px').css('height', '0px').css('overflow', 'hidden').css('display', 'none');
    $('.anaquel-bar').css('overflow', 'hidden').css('height', '55lvh');
    setTimeout(() => {
      $('.anaquel-bar').children().remove();
      setTimeout(() => {
        $('.anaquel-bar').append(`<h1 class="pt-5 mt-3">Analizando Foto</h1>`);
        $('.anaquel-bar').append(`<h6 class="pb-5 mb-5">(Esto podría tomar un tiempo)</h6>`);
        $('.anaquel-bar').append(`
        <div class="banter-loader pt-2" style="scale: 3;">
  ...
  <div class="banter-loader__box"></div>
        ...
        $('.anaquel-bar').css('display', 'flex').css('justify-content', 'start');
    ...
    canvas.toBlob(blob => {
      $.ajax({
        ...
        type: 'POST',
        data: JSON.stringify({ nombre: "1,2,3,4", imagen: blob }),
